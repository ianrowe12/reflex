    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h1 className="font-headline text-xl font-bold text-text-primary">Inventory Management</h1>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-surface-border">
                  <th className="text-left px-4 py-3 text-[10px] font-headline uppercase tracking-wider text-text-muted">Slate</th>
                  <th className="text-right px-4 py-3 text-[10px] font-headline uppercase tracking-wider text-text-muted">Barrels</th>
                  <th className="text-right px-4 py-3 text-[10px] font-headline uppercase tracking-wider text-text-muted">Days of Supply</th>
                </tr>
              </thead>
              <tbody>
                {crudeSlates.map((row, i) => (
                  ...
                  >
                    <td className="px-4 py-2.5 text-sm font-body text-text-primary">{row.slate}</td>
                    <td className="px-4 py-2.5 text-right font-mono text-xs text-text-primary">{row.barrels.toLocaleString()}</td>
                    <td className="px-4 py-2.5 text-right font-mono text-xs text-text-primary">{row.daysOfSupply.toFixed(1)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-surface-border">
                  ...
                  <th className="text-right px-4 py-3 text-[10px] font-headline uppercase tracking-wider text-text-muted">Ready (bbl)</th>
                  <th className="text-right px-4 py-3 text-[10px] font-headline uppercase tracking-wider text-text-muted">Ullage (bbl)</th>
                ...
                {productReady.map((row, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5 text-right font-mono text-xs text-text-primary">{row.barrels.toLocaleString()}</td>
                      <td className="px-4 py-2.5 text-right font-mono text-xs text-text-primary">{row.ullageRemaining.toLocaleString()}</td>
      ...
      {/* Tank Farm Overview */}
      <div>
        <h2 className="text-xs font-headline uppercase tracking-wider text-text-muted font-medium mb-3">Tank Farm Overview</h2>
        <div className="grid grid-cols-4 gap-3">
          {tanks.map((tank) => (
            <div
              key={tank.id}
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-mono font-semibold text-text-primary">{tank.id}</span>
                <span
              ...
              <p className="text-xs font-body text-text-secondary mb-2">{tank.product}</p>
              {/* Fill level bar */}
              <div className="w-full h-2 bg-surface-border-subtle rounded-full mb-2">
                <div
              ...
              <div className="flex justify-between text-xs mb-1">
                ...
                <span className="font-mono text-accent font-semibold">{tank.fillPct}%</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="font-body text-text-muted">Temperature</span>
                <span className="font-mono text-text-primary">{tank.temp} °F</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div>
        <h2 className="text-xs font-headline uppercase tracking-wider text-text-muted font-medium mb-3">Product Inventory</h2>
        <div className="bg-surface-card rounded border border-surface-border shadow-card">
          <table className="w-full">
            <thead>
              <tr className="border-b border-surface-border">
                <th className="text-left px-4 py-3 text-[10px] font-headline uppercase tracking-wider text-text-muted">Product</th>
                <th className="text-left px-4 py-3 text-[10px] font-headline uppercase tracking-wider text-text-muted">Grade</th>
                <th className="text-right px-4 py-3 text-[10px] font-headline uppercase tracking-wider text-text-muted">Volume (bbl)</th>
                <th className="text-center px-4 py-3 text-[10px] font-headline uppercase tracking-wider text-text-muted">Tanks</th>
                <th className="text-left px-4 py-3 text-[10px] font-headline uppercase tracking-wider text-text-muted">Quality</th>
                <th className="text-left px-4 py-3 text-[10px] font-headline uppercase tracking-wider text-text-muted">Last Updated</th>
              </tr>
            </thead>
            <tbody>
              {inventory.map((row, i) => (
                ...
                >
                  <td className="px-4 py-2.5 text-sm font-body text-text-primary">{row.product}</td>
                  <td className="px-4 py-2.5 text-sm font-body text-text-secondary">{row.grade}</td>
                  <td className="px-4 py-2.5 text-right font-mono text-xs text-text-primary">{row.volume.toLocaleString()}</td>
                  <td className="px-4 py-2.5 text-center font-mono text-xs text-text-primary">{row.tanks}</td>
                  <td className="px-4 py-2.5">
                    <span
                      className={`text-[10px] font-headline uppercase tracking-wider px-1.5 py-0.5 rounded-full ${
                        row.quality === "On-Spec"
                  ...
                  <td className="px-4 py-2.5 font-mono text-xs text-text-muted">{row.updated}</td>
                </tr>
              ))}
            </tbody>
          </table>
